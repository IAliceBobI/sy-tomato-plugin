// 同步块引擎（legacyclean □2 自 LinkBox.ts 拆出）：传播（doSync 链）、冲突裁决（syncFromBlock）、
// 全库巡检（scanAllGroups）、组状态（getGroupState）与哈希基线工具。纯逻辑零 svelte 依赖——
// UI 入口（showSyncBlocks 对话框 / addBar 徽标）与命令注册留在 LinkBox.ts，经本层导出接线。
// 设计文档节号（§4.4 决策树等）见原 LinkBox.ts 注释，随函数体原样保留。
import { currentProtyle } from "./libs/Events";
import * as gconst from "./libs/gconst";
import { PROTYLE_WYSIWYG_SELECT } from "./libs/gconst";
import { setAttribute, siyuan } from "./libs/utils";
import * as utils from "./libs/utils";
import { findElementByAttr } from "./libs/listUtils";
import { linkBoxSyncRemapChildID, linkBoxSyncScanDeep } from "./libs/stores";
...
export function doSync(ops: gconst.DoOperation) {
    if (!ops?.id) return;
...
export async function scanAllGroups() {
...
export function findParentSuper(div: HTMLElement) {
