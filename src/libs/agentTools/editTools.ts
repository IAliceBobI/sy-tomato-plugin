...
export function createEditTool(env: ToolEnv): ToolDefinition | null {
    ...
    return {
        name: "edit",
        config: objectSchema(
            "编辑用户文档里的一个块（改写/替换内容）。会先给用户看修改预览，用户确认后才真正写入，"
            + "被拒绝时请勿原样重试，换方案或直接把新内容贴在回答里。blockID 从【用户当前文档】"
...
export function createRunJsTool(env: ToolEnv): ToolDefinition | null {
    ...
    return {
        ...
        config: objectSchema(
            ...
            + "fetch、console；无 import/require；5 秒超时）。代码会先展示给用户，用户确认后才执行。"
            + "写法约定：写成 async 代码体，return 的值作为结果返回（自动 JSON 序列化）。",
            {
...
/** 面板专属装配：通用四工具之外的写/执行面（依赖 env 可选能力，缺则自动瘦身） */
export function createPanelOnlyTools(env: ToolEnv): ToolDefinition[] {
    return [createEditTool(env), createRunJsTool(env)].filter((t): t is ToolDefinition => !!t);
}
