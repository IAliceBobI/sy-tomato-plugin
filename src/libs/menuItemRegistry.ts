// 「右键菜单管理」注册表（□4 起，三期 2026-09-08 归位收官）：列出插件注册的右键菜单项。
// 2026-09-03 瘦身（方案 A 拍板）：有独立开关 store 的项在功能主卡里本就是同一数据双视图，
// 全数移除——各回各家（曾覆盖 13 组：思维导线/块关系图/悬浮球/批注/闪卡/互链与引用/
// 数据库反链/智能问答/图片/阅读点/日记/前缀文档树/块配对浮条/文档整理）。
// 2026-09-03 归位补刀（用户验货拍板）：白/黑名单两项语义严格属于导出工作空间，显隐开关
...
/** 勾选态三层合成纯函数（ConfDocs 文档树工具卡 / ConfExport 导出卡 / ConfAnno 批注卡共用，
 *  勿在组件层复制）：隐藏集优先（藏了就 false）→ master 总开关 → 独立 store（无 store 默认 true）。
...
/** 文档树工具卡消费（2026-09-08 三期归位）：exportFiles 4 个文档树右键菜单项——原通用域
 * 「右键菜单管理」卡末代残留（开关与功能分离的治理起点），现开关跟功能走迁入 ConfDocs
 * 文档树工具卡。无独立 store，显隐走 hiddenMenuItems 隐藏集（机制同 EXPORT/ANNO 两卡）。 */
export const DOCTREE_CARD_MENU_ITEMS: ManagedMenuItem[] = [
