<script lang="ts">
    // 块配对工具域（confgather 2026-09-15，bear 拍板做法丁）：块配对工具卡整卡自「块编辑」域
    // 裁出独立成域，命令开关域「互链与引用」+「长内容工具」两族同期合并为「块配对工具」族
    // ——一个引擎一个总开关（pairBarEnabled 管浮条出场+全部命令注册），拆域人为切割，域族
    // 同名后从命令开关域一眼找到家。卡内一行不动，共享样式见 IndexConf.css。
...
<!-- 块配对工具（R5 □1 总开关化）：总开关联动全部命令注册与浮条出场；子选项收进折叠区
     （原生 details，默认收起，搜索命中 searchSettings 自动展开） -->
