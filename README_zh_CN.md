...
如果没有现有文件名包含`剑仙`二字，则创建`剑仙`文件，并转引用。
...
**`相邻概念`：`AA|BB` 的反链的双链栏，会出现：`EE|AA`、`AA|CC`、`BB|DD`、`AA`、`BB` 如果有。（图中红圈）**

**`相关概念`：双链栏除了`相邻概念`的其他双链。**
...
* 底部反链：引入`相关概念`：`AA|BB` 的反链的双链栏，会出现：`AA|CC`、`BB|DD`、`AA`、`BB` 如果有。
* 闪念：规避冲突的bug.
* 闪念：输入框出现上次旧内容的bug。
* 底部反链：移动端bug。
...
* 修复若干bug。
* 底部反链消失后不出现的bug。
* 快捷菜单下架，部分功能移动到新模块：`杂项`，保留AI部分，依然是F10
